...
408: ---
409: 
410: ## ⚠️ 資料異常排除 (Troubleshooting)
411: 
412: ### 1. 「我的最新資料不見了，雲端被舊資料覆蓋了」
413: 這通常發生在多裝置切換時，舊裝置或新裝置未及時下載雲端資料就觸發了上傳所致。**別擔心，Google 試算表都有歷史紀錄。**
414: 
415: **如何救回資料：**
416: 1. 打開您的試算表（IT Class Master Database）。
417: 2. 點選 **「檔案」 (File)** > **「版本紀錄」 (Version history)** > **「查看版本紀錄」 (See version history)**。
418: 3. 在右側時間軸尋找您同步前的那一個版本（通常是資料量較大、時間點較早的版本）。
419: 4. 確認內容正確後，點選該版本並按上方的 **「還原來源版本」 (Restore this version)**。
420: 5. 還原試算表後，回到網頁端重新整理，系統會偵測到雲端有更新（時間戳記較新）並自動 Pull 下載。
421: 
422: ### 2. 「同步狀態顯示紅色 (Error)」
423: - 確認您提供的 GAS URL 是否正確（開頭必須是 `https://script.google.com/...`）。
424: - 確認部署時「誰可以存取」是否設定為 **「所有人」 (Anyone)**。
425: - 若資料量極大（例如學生紀錄超過數千條），可能會發生逾時。請定期清理舊的點名歷史紀錄，或依照系統提示更新最新版的 GAS 代碼以支援分段儲存。
